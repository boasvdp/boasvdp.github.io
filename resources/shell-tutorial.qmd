...
This page lays out the exercises to learn shell scripting. The exercises are also used in the PHM III course for the ECDC fellowship.
...
::: {.callout-note collapse="true" title="Answer" icon="false"}
```bash
for file in sequences/*.fasta  # <1>
do  # <2>
  echo "Processing file: $file"  # <3>
  wc -l "$file" | cut -d' ' -f1  # <4>
done  # <5>
```
1. Finds all files matching the pattern `sequences/*.fasta` and stores them in the variable `$file` one by one
2. Needed in a for loop, indicates the start of repeated commands
3. Prints the file name to the screen 
4. Uses `wc -l` to count the number of lines per file, pipes this into `cut` which then extracts the first field, using a space as separator.
5. Needed in a for loop, indicates the end of the repeated commands
:::
...
- Lessons 6 and 7 of [https://carpentry.library.ucsb.edu/CustomLibraryBash/](https://carpentry.library.ucsb.edu/CustomLibraryBash/)
- Any extra material from [https://carpentries-incubator.github.io/shell-extras/](https://carpentries-incubator.github.io/shell-extras/)
